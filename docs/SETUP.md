...
| Herramienta | Para qué | Instalación (Ubuntu) |
|---|---|---|
| Python 3.11+ | Ejecutar la API | (del sistema / `uv python install`) |
| [uv](https://docs.astral.sh/uv/) | Gestor de dependencias | `curl -LsSf https://astral.sh/uv/install.sh \| sh` |
| Docker | Adaptador `local_docker` | Docker Engine o Docker Desktop |
| [just](https://github.com/casey/just) | Atajos de tareas (opcional) | `sudo snap install just` / paquete del sistema |
| gcloud CLI | Setup de GCP | `sudo snap install google-cloud-cli --classic` |
| AWS CLI | Setup/gestión de AWS (opcional) | `sudo snap install aws-cli --classic` |
| psql | Hacer SQL contra las instancias (data plane) | `sudo apt install postgresql-client` |
...
2. Si usas **Docker Desktop en Linux**, el socket no está en
   `/var/run/docker.sock` sino en `~/.docker/desktop/docker.sock`. Hay que
   indicarlo en `.env`:
   ```
   DOCKER_HOST=unix:///home/<usuario>/.docker/desktop/docker.sock
   ```
3. Con Docker Engine clásico, normalmente no hace falta tocar `DOCKER_HOST`.
...
4. Generar **access keys** (Access Key ID + Secret) para ese usuario.

### 4.2. Credenciales

`boto3` las lee de `~/.aws/credentials`. Crear con `aws configure` o a mano:
...
Crea `~/.config/gcloud/application_default_credentials.json`, que la librería
de Google lee automáticamente. **Marca todos los permisos** en la pantalla de
consentimiento (incluido `cloud-platform`) o fallará.
...
- **Docker**: gratis.
- **AWS RDS**: `db.t3.micro` está en free tier (750 h/mes el primer año).
